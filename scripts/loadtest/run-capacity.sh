...
export_stage() {
  ...
  STAGE_REASON="$reason" STAGE_PASSED="$passed_bool" STAGE_APP_ALIVE="$app_alive_bool" \
    STAGE_DB_ALIVE="$db_alive_bool" python3 - "$out_path" "$rps" "$k6_rc" "$peak_app" "$peak_db" \
    "$http_fail" "$checks" "$flow_errors" <<'PY'
import json, os, sys
path, rps, k6_rc, peak_app, peak_db, http_fail, checks, flow_errors = sys.argv[1:9]
doc = {
  "rps": int(rps),
  ...
  "cpu_limit_pct": float(os.environ.get("CPU_LIMIT_PCT", "80")),
}
...
run_stage() {
  ...
  peak_app="$(peak_cpu_column "$cpu_log" 1)"
  peak_db="$(peak_cpu_column "$cpu_log" 2)"
